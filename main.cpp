#include <iostream>
#include <fstream>
#include <cstring>
#define DEEP 3
using namespace std;

static const unsigned char Cp1251ToCp866[ 128 ] = {

    
};

static const unsigned char Cp1251ToKoi8R[ 128 ] = {

    
};

static const unsigned char Cp866ToCp1251[ 128 ] = {

    
};

static const unsigned char Cp866ToKoi8R[ 128 ] = {

    
};

static const unsigned char Koi8RToCp1251[ 128 ] = {

    
};

static const unsigned char Koi8RToCp866[ 128 ] = {

    
};

class Decryptor{
    int frequency[128]; // встречаемость по индексу + 128
    int amount; // колличество
    int* sums[DEEP];
    string fileName;
    ifstream iFile;
    ofstream oFile;
public:
    Decryptor(){
    int i, j, el = 1;
    for ( i = 0; i< 128; i++)
    frequency[ i ] = 0;
    amount = 0;
    for (j=0;j<DEEP;j++){
        sums[j] = new int[el];
        for(i=0;i < el; i++)
            sums[j][i]=0;
        el = el * 6;
    }
    }

    int start(string file){
        fileName = file;
        if (openFile() == 1){
            cout << "file is not finded" << endl;
            return 1;
        }
        if (checkASCII()==1){
            cout << "file don have not ASCII symbols" << endl;
            return 1;
        }

        countAmount();
        unsigned char firstTable[128];
        for ( int i = 0; i < 128; i++)
            firstTable[ i ] = i|0x80;
        researh(1,0,firstTable);
        return 0;
    }

    // проверка на содержание не аски символов
    int checkASCII(){
        for (int i = 0; i < 128; i++)
            if(frequency[i]!=0) return 0;
        return 1;
    }
    // составление статистики встречаемости символов в файле
    int  openFile(){
        char c;
        iFile.open(fileName);
        if(!iFile) return 1; // файл отсутствует
        while(!iFile.eof()){
            iFile.get(c);
            if(c&0x80) frequency[ c&0x7f ]++;
        }
        return 0;
    }

    void countAmount(){
        for (int i = 0; i < 128; i++)
            amount+=frequency[ i ];
    }
    // либо смещение либо поиск для в каждом вызове свободных мест  слева для записи
    // работает ли должно быть нечто содержащее суммуж
    int researh(int level,int plase, unsigned char (&baseTables)[128]){
        if (level == DEEP) return 1;
        unsigned char tables[6][128];
        int i = 0, j = 0;
        long sumOne[6]={0,0,0,0,0,0};
        for (i = 0;i < 128 ; i++){
            tables[0][i] = Cp1251ToCp866[ baseTables[i] &0x7f ];
            tables[1][i] = Cp1251ToKoi8R[ baseTables[i] &0x7f ];
            tables[2][i] = Cp866ToCp1251[ baseTables[i] &0x7f ];
            tables[3][i] = Cp866ToKoi8R [ baseTables[i] &0x7f ];
            tables[4][i] = Koi8RToCp1251[ baseTables[i] &0x7f ];
            tables[5][i] = Koi8RToCp866 [ baseTables[i] &0x7f ];
        }
        for (j = 0; j < 6; j++){
            for (i = 0; i < 128;i++)
                sumOne[j]+=tables[j][i]*frequency[i];

            sums[level][plase+j] = sumOne[j];
        }

        for (j = 0; j < 6; j++)
        researh(level+1,plase+j,tables[j]);
    }

};

int main(int argc, char *argv[])
{
    Decryptor decryptor;
    cout << "Hello World!" << endl;
    return 0;
}
